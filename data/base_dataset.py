...
def get_transform(
    ...
    if "resize" in opt.preprocess:
        ...
        transform_list.append(
            transforms.Lambda(
                lambda img: __scale_longside(img, opt.load_size, opt.crop_size, method)
            )
        )
        ...
        if params is None or "crop_pos" not in params:
            transform_list.append(
                transforms.RandomCrop(
                    opt.crop_size, padding=opt.preprocess_crop_padding
                )
            )
        else:
            transform_list.append(
    ...
    transform_list.append(
        transforms.Lambda(lambda img: __make_power_2(img, base=16, method=method))
    )
